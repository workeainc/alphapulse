# Enhanced AlphaPlus System Setup Script (PowerShell)
...
function Start-DockerServices {
    ...
    try {
        ...
        $runningServices = docker-compose -f $DockerComposeFile ps --format json | ConvertFrom-Json
        if ($runningServices.Count -gt 0) {
            Write-Warn "Stopping existing services..."
...
function Test-System {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200) {
            ...
            Write-Warn "⚠️ API health check returned status: $($response.StatusCode)"
        }
        ...
        try {
            ...
            Write-Warn "⚠️ Cache statistics endpoint not available yet"
        }
        ...
        Write-Warn "⚠️ System tests failed: $($_.Exception.Message)"
        return $false
    }
}
...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Usage: .\setup_enhanced_system.ps1 [options]" -ForegroundColor White
    Write-Host ""
