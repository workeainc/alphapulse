...
class AdvancedPatternDetector:
    ...
    async def _is_bullish_engulfing(self, df: pd.DataFrame, i: int) -> bool:
        """Check if bars i-1 and i form a bullish engulfing pattern"""
        try:
            ...
            # Current bar should engulf previous bar
    ...
    async def _is_bearish_engulfing(self, df: pd.DataFrame, i: int) -> bool:
        """Check if bars i-1 and i form a bearish engulfing pattern"""
        try:
            prev_open = df.iloc[i-1]['open']
            prev_close = df.iloc[i-1]['close']
            curr_open = df.iloc[i]['open']
