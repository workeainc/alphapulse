...
class VolumeAnalyzer:
    ...
    def _update_vwap(self, symbol: str, volume_data: dict):
        """Update VWAP calculation for real-time data"""
        try:
            ...
            if volume > 0 and price > 0:
                self.vwap_cache[symbol]['cumulative_volume'] += volume
                self.vwap_cache[symbol]['cumulative_price_volume'] += price * volume
                
                if self.vwap_cache[symbol]['cumulative_volume'] > 0:
                    self.vwap_cache[symbol]['vwap'] = (
                        self.vwap_cache[symbol]['cumulative_price_volume'] / 
                        self.vwap_cache[symbol]['cumulative_volume']
                    )
    ...
    def get_volume_profile_realtime(self, symbol: str) -> dict:
        """Get real-time volume profile for a symbol"""
        try:
            if symbol not in self.volume_buffer or not self.volume_buffer[symbol]:
                return {}
            ...
            for point in volume_data:
                price = round(point['price'], 2)  # Round to 2 decimal places
                if price not in price_volume:
                    price_volume[price] = 0
                price_volume[price] += point['volume']
    ...
    async def analyze_volume_patterns(self, df: pd.DataFrame, symbol: str = "", timeframe: str = "") -> List[VolumePattern]:
        """
        Analyze volume patterns in the given DataFrame and return detected volume patterns
        ...
        try:
            ...
            # Detect different types of volume patterns
            patterns = []
            
            # 1. Volume Spike Detection
            if self._detect_volume_spike(df, volume_analysis):
                patterns.append(self._create_volume_spike_pattern(df, volume_analysis, symbol, timeframe))
            
            # 2. Volume Divergence Detection
            if self._detect_volume_divergence(df, volume_analysis):
                patterns.append(self._create_volume_divergence_pattern(df, volume_analysis, symbol, timeframe))
            
            # 3. Volume Climax Detection
            if self._detect_volume_climax(df, volume_analysis):
                patterns.append(self._create_volume_climax_pattern(df, volume_analysis, symbol, timeframe))
            
            # 4. Volume Dry-up Detection
            if self._detect_volume_dry_up(df, volume_analysis):
                patterns.append(self._create_volume_dry_up_pattern(df, volume_analysis, symbol, timeframe))
            
            # 5. Volume Breakout Detection
            if self._detect_volume_breakout(df, volume_analysis):
                patterns.append(self._create_volume_breakout_pattern(df, volume_analysis, symbol, timeframe))
            
            # 6. Accumulation/Distribution Detection
            ...
            for breakdown_idx in breakdown_candles:
                if breakdown_idx >= len(lows) - 3:  # Recent breakdown
                    ...
                    if recovery_found:
                        ...
                        breakdown_depth = (support_level - lows[breakdown_idx]) / support_level
                        recovery_speed = j - breakdown_idx
