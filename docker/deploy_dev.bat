@echo off
REM AlphaPulse Development Deployment Script for Windows
REM This script deploys the entire AlphaPulse system using Docker Compose
...
docker-compose -f docker-compose.development.yml exec -T postgres pg_isready -U alpha_emon -d alphapulse >nul 2>&1
if errorlevel 1 (
...
REM Check Redis
docker-compose -f docker-compose.development.yml exec -T redis redis-cli ping >nul 2>&1
if errorlevel 1 (
...
if errorlevel 1 (
    echo [WARNING] Monitoring Dashboard is still starting up...
) else (
    echo [SUCCESS] Monitoring Dashboard is ready
)
...
if errorlevel 1 (
    ...
    echo [WARNING] You can check the logs with: docker-compose -f docker-compose.development.yml logs -f
) else (
    curl -f http://localhost:3000 >nul 2>&1
    if errorlevel 1 (
        echo [WARNING] ⚠️  Some services may still be starting up. Please wait a few more minutes.
