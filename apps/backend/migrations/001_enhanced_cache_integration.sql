-- Enhanced Cache Integration Migration
-- This migration creates all necessary tables and optimizations for the enhanced cache system
...
SELECT create_hypertable('enhanced_market_data', 'timestamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
CREATE INDEX IF NOT EXISTS idx_enhanced_market_data_symbol_timeframe_timestamp ON enhanced_market_data (symbol, timeframe, timestamp DESC);
...
SELECT create_hypertable('market_conditions', 'timestamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
SELECT create_hypertable('data_quality_metrics', 'timestamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
SELECT create_hypertable('data_anomalies', 'timestamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => TRUE);
...
-- Create TimescaleDB hypertable for cache performance metrics
SELECT create_hypertable('cache_performance_metrics', 'timestamp', chunk_time_interval => INTERVAL '1 hour', if_not_exists => TRUE);

-- Create indexes for cache performance metrics
CREATE INDEX IF NOT EXISTS idx_cache_performance_timestamp ON cache_performance_metrics (timestamp);
CREATE INDEX IF NOT EXISTS idx_cache_performance_type ON cache_performance_metrics (cache_type);
CREATE INDEX IF NOT EXISTS idx_cache_performance_operation ON cache_performance_metrics (operation_type);
CREATE INDEX IF NOT EXISTS idx_cache_performance_symbol ON cache_performance_metrics (symbol);

-- WebSocket Performance Metrics Table
CREATE TABLE IF NOT EXISTS websocket_performance_metrics (
    id BIGSERIAL PRIMARY KEY,
    timestamp TIMESTAMPTZ NOT NULL,
    active_connections INTEGER NOT NULL,
    total_messages_sent INTEGER NOT NULL,
    avg_latency_ms NUMERIC(8,3) NOT NULL,
    errors_count INTEGER NOT NULL DEFAULT 0,
    queue_sizes JSONB,
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
);

-- Create TimescaleDB hypertable for WebSocket performance metrics
SELECT create_hypertable('websocket_performance_metrics', 'timestamp', chunk_time_interval => INTERVAL '1 hour', if_not_exists => TRUE);

-- Create indexes for WebSocket performance metrics
CREATE INDEX IF NOT EXISTS idx_websocket_performance_timestamp ON websocket_performance_metrics (timestamp);
CREATE INDEX IF NOT EXISTS idx_websocket_performance_connections ON websocket_performance_metrics (active_connections);
...
DROP TRIGGER IF EXISTS update_pattern_detections_updated_at ON pattern_detections;
...
-- Set compression policies for better performance
...
ALTER TABLE market_conditions SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'symbol,timeframe',
    timescaledb.compress_orderby = 'timestamp DESC'
);
...
ALTER TABLE data_anomalies SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'symbol,timeframe',
    timescaledb.compress_orderby = 'timestamp DESC'
);
...
ALTER TABLE cache_performance_metrics SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'cache_type,operation_type',
    timescaledb.compress_orderby = 'timestamp DESC'
);

ALTER TABLE websocket_performance_metrics SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'active_connections',
    timescaledb.compress_orderby = 'timestamp DESC'
);
...
SELECT add_compression_policy('market_conditions', INTERVAL '7 days');
SELECT add_compression_policy('data_quality_metrics', INTERVAL '7 days');
SELECT add_compression_policy('data_anomalies', INTERVAL '7 days');
SELECT add_compression_policy('cache_performance_metrics', INTERVAL '7 days');
SELECT add_compression_policy('websocket_performance_metrics', INTERVAL '7 days');

-- Add retention policies (keep data for 90 days)
...
SELECT add_retention_policy('data_anomalies', INTERVAL '90 days');
SELECT add_retention_policy('cache_performance_metrics', INTERVAL '30 days');
SELECT add_retention_policy('websocket_performance_metrics', INTERVAL '30 days');

-- Verify tables were created
SELECT table_name, is_hypertable 
FROM timescaledb_information.hypertables 
WHERE table_name IN ('enhanced_market_data', 'market_conditions', 'data_quality_metrics', 'data_anomalies', 'cache_performance_metrics', 'websocket_performance_metrics');

-- Show table structure
\d enhanced_market_data;
\d pattern_detections;
\d signal_history;
\d performance_metrics;
\d confidence_scores;
\d market_conditions;
\d data_quality_metrics;
\d data_anomalies;
\d cache_performance_metrics;
\d websocket_performance_metrics;
