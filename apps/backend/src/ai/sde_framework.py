...
# Import consensus manager
from .consensus_manager import ConsensusManager, ConsensusResult as ConsensusManagerResult
...
class ConsensusResult:
    """Model consensus result (DEPRECATED - use ConsensusManagerResult from consensus_manager.py)"""
    ...
    # NEW: For compatibility with updated consensus_manager
    consensus_probability: float = 0.0
    consensus_confidence: float = 0.0
    min_agreeing_heads: int = 4
    total_heads: int = 0
...
class SDEFramework:
    ...
    def __init__(self, db_pool: asyncpg.Pool):
        ...
        # Initialize consensus manager (UPDATED - uses specification-compliant consensus mechanism)
        ...
        # Consensus configuration (UPDATED - per specification)
        # NOTE: These are now managed by ConsensusManager, kept here for backwards compatibility
        self.consensus_config = {
            'min_agreeing_heads': 4,  # 44% rule (4 out of 9 heads)
    ...
    async def check_model_consensus(self, model_results: List[ModelHeadResult]) -> ConsensusResult:
        """
        Check if model heads reach consensus using the specification-compliant ConsensusManager
        
        UPDATED: Now uses ConsensusManager with:
        - 44% rule (4 out of 9 heads)
        - Probability ≥ 0.60, Confidence ≥ 0.70
        - Consensus Confidence with agreement + strength bonuses
        - Minimum 0.65 consensus confidence gate
        """
        try:
            ...
            await self._store_consensus_tracking(
                ...
                consensus_manager_result.consensus_confidence  # Use consensus_confidence, not consensus_score
            )
            
            # Convert ConsensusManagerResult to ConsensusResult for backwards compatibility
            return ConsensusResult(
                ...
                consensus_score=consensus_manager_result.consensus_score,  # Backwards compatibility
                ...
                agreeing_heads=consensus_manager_result.agreeing_heads,
                disagreeing_heads=consensus_manager_result.disagreeing_heads,
                confidence_threshold=consensus_manager_result.confidence_threshold,
                min_agreeing_heads=consensus_manager_result.min_agreeing_heads,
                total_heads=consensus_manager_result.total_heads
            )
            ...
            return ConsensusResult(
                ...
                min_agreeing_heads=self.consensus_config['min_agreeing_heads'],
                total_heads=len(model_results) if model_results else 0
            )
    ...
    async def _create_head_d_rulebased(self, 
                                     ...
                                     symbol: str,
                                     timeframe: str) -> ModelHeadResult:
        """Create Head D: Rule-based analysis with 60+ pattern detection
        
        Delegates to RuleBasedHead which performs:
        - 60+ candlestick pattern detection (TA-Lib)
        - Chart pattern detection (H&S, double tops, triangles, flags)
        - Support/resistance level checking
        - Volume confirmation
        """
        try:
            # Import and use the actual RuleBasedHead
            from .model_heads import RuleBasedHead
            
            # Create instance (will lazy-load pattern detectors)
            rule_based_head = RuleBasedHead()
            ...
            # Delegate to the actual head implementation
            result = await rule_based_head.analyze(market_data, analysis_results)
            
            logger.debug(f"✅ Head D (Rule-based): {result.direction.value} @ {result.confidence:.2f} conf - {result.reasoning[:100]}")
            
            return result
            
        except Exception as e:
            logger.error(f"❌ Head D creation failed: {e}")
            return self._create_default_head(ModelHead.HEAD_D, f"Rule-based analysis failed: {str(e)}")
    ...
    async def _calculate_zone_score(self, df: pd.DataFrame, symbol: str, timeframe: str) -> float:
        ...
        try:
            ...
            if df.empty or 'high' not in df.columns or 'low' not in df.columns or 'close' not in df.columns:
                logger.warning(f"DataFrame missing required columns for zone score calculation")
                return 5.0
            ...
            distance_to_high = (high_20 - current_price) / current_atr if current_atr > 0 else 100
            distance_to_low = (current_price - low_20) / current_atr if current_atr > 0 else 100
    ...
    async def _calculate_htf_bias(self, df: pd.DataFrame, symbol: str, timeframe: str) -> float:
        ...
        try:
            ...
            if df.empty or 'close' not in df.columns:
                logger.warning(f"DataFrame missing required columns for HTF bias calculation")
                return 5.0
            ...
            ema_50_current = ema_50.iloc[-1] if len(ema_50) > 0 else current_price
    ...
    async def _calculate_trigger_quality(self, df: pd.DataFrame, symbol: str, timeframe: str) -> float:
        ...
        try:
            # Check if DataFrame has required columns
            if df.empty or 'close' not in df.columns:
                logger.warning(f"DataFrame missing required columns for trigger quality calculation")
                return 5.0
    ...
    async def _calculate_ob_alignment(self, df: pd.DataFrame, symbol: str, timeframe: str) -> float:
        ...
        try:
            ...
            if df.empty or 'volume' not in df.columns:
                logger.warning(f"DataFrame missing required columns for OB alignment calculation")
                return 5.0
    ...
    async def assess_execution_quality(self, market_data: Dict[str, Any]) -> ExecutionQuality:
        """Assess execution quality for the signal"""
        try:
            # Extract market data
            current_price = market_data.get('current_price', 0.0)
            spread_atr_ratio = market_data.get('spread_atr_ratio', 0.1)
            impact_cost = market_data.get('impact_cost', 0.05)
            atr_percentile = market_data.get('atr_percentile', 50.0)
            
            # Calculate execution score based on market conditions
            execution_score = 0.0
            ...
            # ATR percentile (moderate volatility is better)
            ...
            return ExecutionQuality(
                spread_ok=spread_atr_ratio < 0.1,
                impact_cost=impact_cost,
                volatility_regime="medium" if 30 <= atr_percentile <= 70 else "high" if atr_percentile > 70 else "low",
                liquidity_score=min(1.0, execution_score),
                execution_score=min(10.0, execution_score * 10)
            )
    ...
    async def check_news_blackout(self, symbol: str, timestamp: datetime = None) -> Any:
        """Check if there's a news blackout for the symbol"""
        try:
            if timestamp is None:
                timestamp = datetime.now()
            
            # Check for recent news events that might cause blackout
            async with self.db_pool.acquire() as conn:
                # Check for high-impact news in the last hour
                recent_news = await conn.fetchval("""
                    SELECT COUNT(*) FROM market_intelligence 
                    WHERE symbol = $1 AND timestamp > $2 - INTERVAL '1 hour'
                    AND event_type = 'news' AND impact_score > 0.7
                """, symbol, timestamp)
                
                if recent_news and recent_news > 0:
                    return type('NewsBlackout', (), {
                        'blackout_active': True,
                        'reason': f'High-impact news detected ({recent_news} events)',
                        'timestamp': timestamp
                    })()
                else:
                    return type('NewsBlackout', (), {
            ...
            return type('NewsBlackout', (), {
                'blackout_active': False,
                'reason': f'Error checking news: {e}',
                'timestamp': timestamp or datetime.now()
            })()
    
    async def check_signal_limits(self, symbol: str, account_id: str) -> Any:
        """Check signal limits for account and symbol"""
        try:
            async with self.db_pool.acquire() as conn:
                ...
                return type('SignalLimits', (), {
                    'account_limit_reached': account_signals >= 10,  # Max 10 signals per hour per account
                    'symbol_limit_reached': symbol_signals >= 5,    # Max 5 signals per hour per symbol
    ...
    def _calculate_atr(self, df: pd.DataFrame, period: int) -> pd.Series:
        ...
        try:
            # Check if DataFrame has required columns
            if df.empty or 'high' not in df.columns or 'low' not in df.columns or 'close' not in df.columns:
                logger.warning(f"DataFrame missing required columns for ATR calculation")
