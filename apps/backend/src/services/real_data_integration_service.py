...
class RealDataIntegrationService:
    ...
    async def get_ohlcv_data(self, symbol: str, timeframe: str = "1h", limit: int = 200) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            async with self.db_connection.get_connection() as conn:
                rows = await conn.fetch(query, symbol.upper(), timeframe, limit)
                ...
                if rows:
                    ...
                    self.logger.debug(f"✅ Fetched {len(ohlcv_list)} OHLCV records for {symbol} {timeframe}")
                    return ohlcv_list
