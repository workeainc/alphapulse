...
-- Stores candlestick data from WebSocket and REST API sources
...
CREATE TABLE IF NOT EXISTS ohlcv_data (
    ...
    source VARCHAR(20) DEFAULT 'websocket', -- 'websocket', 'rest_api', 'historical'
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_ohlcv_unique ON ohlcv_data (symbol, timeframe, timestamp);
...
CREATE INDEX IF NOT EXISTS idx_orderbook_symbol_timestamp ON order_book_data (symbol, timestamp);
CREATE INDEX IF NOT EXISTS idx_orderbook_timestamp ON order_book_data (timestamp);
...
CREATE TABLE IF NOT EXISTS technical_indicators (
    ...
    calculation_method VARCHAR(50), -- 'sma', 'ema', 'rsi', 'macd', etc.
    created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL
);
...
CREATE INDEX IF NOT EXISTS idx_indicators_symbol_name_timestamp ON technical_indicators (symbol, indicator_name, timestamp);
...
CREATE TABLE IF NOT EXISTS support_resistance_levels (
    ...
    symbol VARCHAR(20) NOT NULL,
    timeframe VARCHAR(10) NOT NULL,
    level_type VARCHAR(20) NOT NULL, -- 'support', 'resistance', 'psychological'
    price_level NUMERIC(20,8) NOT NULL,
    strength NUMERIC(4,3) NOT NULL, -- 0.0 to 1.0
...
CREATE INDEX IF NOT EXISTS idx_sr_symbol_timeframe ON support_resistance_levels (symbol, timeframe);
CREATE INDEX IF NOT EXISTS idx_sr_level_type ON support_resistance_levels (level_type);
CREATE INDEX IF NOT EXISTS idx_sr_price_level ON support_resistance_levels (price_level);
CREATE INDEX IF NOT EXISTS idx_sr_active ON support_resistance_levels (is_active);
...
CREATE INDEX IF NOT EXISTS idx_ohlcv_5m_symbol_bucket ON ohlcv_5m (symbol, bucket);
CREATE INDEX IF NOT EXISTS idx_ohlcv_15m_symbol_bucket ON ohlcv_15m (symbol, bucket);
CREATE INDEX IF NOT EXISTS idx_ohlcv_1h_symbol_bucket ON ohlcv_1h (symbol, bucket);
...
DECLARE
    rec RECORD;
    sma_20_value NUMERIC(20,8);
    sma_50_value NUMERIC(20,8);
    rsi_14 NUMERIC(20,8);
    atr_14 NUMERIC(20,8);
BEGIN
    -- Calculate SMA 20
    SELECT AVG(close) INTO sma_20_value
    FROM (
    ...
    -- Calculate SMA 50
    SELECT AVG(close) INTO sma_50_value
    FROM (
        SELECT close FROM ohlcv_data 
        WHERE symbol = p_symbol AND timeframe = p_timeframe
    ...
    VALUES 
        (p_symbol, p_timeframe, NOW(), 'SMA_20', sma_20_value, 'simple_moving_average'),
        (p_symbol, p_timeframe, NOW(), 'SMA_50', sma_50_value, 'simple_moving_average')
