-- Final fix for function syntax errors
...
BEGIN
    ...
    -- Add TimescaleDB retention policy using dynamic SQL
    EXECUTE format('SELECT add_retention_policy(%L, INTERVAL %L)', p_table_name, p_retention_days || ' days');
...
BEGIN
    ...
    -- Add TimescaleDB compression policy using dynamic SQL
    EXECUTE format('SELECT add_compression_policy(%L, INTERVAL %L)', p_table_name, p_compress_after_days || ' days');
...
BEGIN
    ...
    WHERE le.started_at >= NOW() - (p_days_back || ' days')::INTERVAL
...
VALUES (NULL, 'migration', 'system', 'success', 
        '{"migration_version": "075", "migration_name": "final_function_fix", "fixes_applied": ["interval_syntax"]}');

DO $$
BEGIN
    RAISE NOTICE 'All function syntax errors fixed successfully';
END $$;
